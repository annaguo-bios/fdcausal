...
ATT.TMLE.ratio <- function(a,data,treatment, mediators, outcome, covariates,
  ...
  if (crossfit==T){ # use cross fitting
    ...
    if (boundedsubmodelY & estimator=='tmle' & !Y.binary){
      or_fit <- CV.SuperLearner(Y=(Y-min(Y))/(max(Y)-min(Y)), X=data.frame(M,A=A,X), family = Y.family, V = K, SL.library = lib, control = list(saveFitLibrary=T),saveAll = T)
    }else{
    ...
    if (boundedsubmodelY & estimator=='tmle' & !Y.binary){
      or_fit <- SuperLearner(Y=(Y-min(Y))/(max(Y)-min(Y)), X=data.frame(M,A=A,X), family = Y.family, SL.library = lib)
    }else{
    ...
    if (boundedsubmodelY & estimator=='tmle' & !Y.binary){
      or_fit <- glm(as.formula(formulaY), data=data.frame(Y=(Y-min(Y))/(max(Y)-min(Y)),M,A=A,X), family = Y.family)
    }else{
      or_fit <- glm(as.formula(formulaY), data=data.frame(Y=Y,M,A=A,X), family = Y.family)
    }
  ...
  if (boundedsubmodelY & estimator=='tmle' & !Y.binary){
    or_pred[or_pred<0.005] <- 0.005; or_pred[or_pred>0.995] <- 0.995
    or_pred_alt[or_pred_alt<0.005] <- 0.005; or_pred_alt[or_pred_alt>0.995] <- 0.995
  }
  ...
  if(estimator=='tmle'){
    ...
    offset.Y <- if(Y.binary|boundedsubmodelY){qlogis(or_pred_alt)}else{or_pred_alt}
    ...
    if(Y.binary|boundedsubmodelY){model.Y <- glm(Y ~ offset(offset.Y) + clever_coef.Y -1, weights=(A==alt)*1, family=binomial() )}else{model.Y <- glm( Y ~ offset(offset.Y)+1, weights=or_weight, family = gaussian())}
    ...
    # update E(Y|M,alt,X)
    or_pred_alt_updated <- if(Y.binary|boundedsubmodelY){plogis(offset.Y + eps.Y*clever_coef.Y)}else{or_pred_alt + eps.Y}
