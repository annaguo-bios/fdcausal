...
TMLE.all <- function(a,data,treatment, mediators, outcome, covariates,
                     ...
                     estimator=c('onestep','tmle'),boundedsubmodelY=F){
  ...
  if (length(mediators)==1 & all(unlist(M) %in% 0:1)){ # METHOD 1- BINARY: univariate binary mediator
    ...
    if(length(estimator)==2){
      ...
      tmle.binary.res1 <- TMLE.binary(a=a,data=data,treatment=treatment, mediator=mediators, outcome=outcome, covariates=covariates,
                                     ...
                                     estimator=estimator[1], boundedsubmodelY=boundedsubmodelY)

      tmle.binary.res2 <- TMLE.binary(a=a,data=data,treatment=treatment, mediator=mediators, outcome=outcome, covariates=covariates,
                                      superlearner=superlearner,crossfit=crossfit, K=K, lib=lib,n.iter=n.iter, cvg.criteria=cvg.criteria,
                                      formulaY=formulaY, formulaA=formulaA, formulaM=formulaM, linkY_binary=linkY_binary, linkA=linkA, linkM_binary=linkM_binary,
                                      truncate_lower=truncate_lower, truncate_upper=truncate_upper,
                                      estimator=estimator[2], boundedsubmodelY=boundedsubmodelY)
      ...
      tmle.binary.res <- TMLE.binary(a=a,data=data,treatment=treatment, mediator=mediators, outcome=outcome, covariates=covariates,
                                     superlearner=superlearner,crossfit=crossfit, K=K, lib=lib,n.iter=n.iter, cvg.criteria=cvg.criteria,
                                     formulaY=formulaY, formulaA=formulaA, formulaM=formulaM, linkY_binary=linkY_binary, linkA=linkA, linkM_binary=linkM_binary,
                                     truncate_lower=truncate_lower, truncate_upper=truncate_upper,
                                     estimator=estimator, boundedsubmodelY=boundedsubmodelY)
    ...
    if (length(mediators)>1){ # np method only allow univariate mediator
      ...
      if (all(Y %in% c(0,1))|boundedsubmodelY){ # binary outcome require iterative update among propensity score, mediator density, and outcome regression. Results can be very unstable.
  ...
  if(length(estimator)==2){
    ...
    tmle.ratio.out1 <- TMLE.ratio(a = a,
                                 data = data,
                                 treatment = treatment,
                                 mediators = mediators,
                                 outcome = outcome,
    ...
    tmle.ratio.out2 <- TMLE.ratio(a = a,
                                  data = data,
                                  treatment = treatment,
                                  mediators = mediators,
                                  outcome = outcome,
