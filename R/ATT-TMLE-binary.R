...
#' @return Function outputs a list containing TMLE/onestep output:
#' \describe{
...
ATT.TMLE.binary <- function(a,data,treatment, mediator, outcome, covariates,
  ...
  if (crossfit==T){ # use cross fitting
    ...
    if (boundedsubmodelY & estimator=='tmle' & !Y.binary){
      or_fit <- CV.SuperLearner(Y=(Y-min(Y))/(max(Y)-min(Y)), X=data_MAX, family = Y.family, V = K, SL.library = lib, control = list(saveFitLibrary=T),saveAll = T)
    }else{
    ...
    if (boundedsubmodelY & estimator=='tmle' & !Y.binary){
      or_fit <- SuperLearner(Y=(Y-min(Y))/(max(Y)-min(Y)), X=data_MAX, family = Y.family, SL.library = lib)
    }else{
    ...
    if (boundedsubmodelY & estimator=='tmle' & !Y.binary){
      or_fit <- glm(as.formula(formulaY), data=data_MAX %>% mutate(Y=(Y-min(Y))/(max(Y)-min(Y)) ), family = Y.family)
    }else{
  ...
  if (boundedsubmodelY & estimator=='tmle' & !Y.binary){
    ...
    or_pred_altm1[or_pred_altm1<0.005] <- 0.005; or_pred_altm1[or_pred_altm1>0.995] <- 0.995
    or_pred_altm0[or_pred_altm0<0.005] <- 0.005; or_pred_altm0[or_pred_altm0>0.995] <- 0.995
