...
The ACE $E(Y^1)-E(Y^0)$ can be estimated via the `estfd()` function as follows:

```{r}

# linkA specifies using "identity" link for fitting the logistic regression of A on X.

cYcM <- estfd(a=c(1,0),data=continuousY_continuousM,
             treatment="A", mediators="M", outcome="Y", covariates="X",
                      estimator='onestep', linkA="identity") 
```

In the above code, `a=c(1,0)` specifies estimating ACE, contrasting treatment level $a=1$ verse $a=0$. `estimator="onestep"` specifies output one-step estimator.
...
The source code for **fdcausal** package is available on GitHub at
...
cYcM <- estfd(a=c(1,0),data=continuousY_continuousM_10dX, treatment="A", mediators="M", outcome="Y", covariates=paste0("X.",1:10), 
...
The density ratio is estimated via estimating $p(A \mid X, M)$ and $\pi(A\mid X)$. $p(A \mid X, M)$ can be estimated via super learn, cross-fitting in conjunction with super learner, or logistic regression. When using logistic regression, `formula_bayes` and `link_bayes` arguments in `estfd()` allow users to specify the formula and link function used in logistic regression.
...
cYcM <- estfd(a=1,data=continuousY_continuousM, treatment="A", mediators="M", outcome="Y", covariates="X", 
...
cYcM <- estfd(a=c(1,0),data=continuousY_continuousM_10dX, treatment="A", mediators="M", outcome="Y", covariates=paste0("X.",1:10), 
             estimator=c('onestep','tmle'), mediator.method = "bayes")
